...
class AuthRepositoryImplTest {
    ...
    fun `register success saves tokens and returns auth`() = runBlocking {
        ...
        val result = repository.register(RegisterRequest("u", "e", "p", "n"))
    ...
    fun `login success saves tokens`() = runBlocking {
        ...
        val result = repository.login(LoginRequest("u", "p"))
    ...
    fun `logout clears tokens`() = runBlocking {
        ...
        val result = repository.logout(LogoutRequest("jwt", "rt"))
    ...
    fun `refreshToken success saves new tokens`() = runBlocking {
        ...
        val result = repository.refreshToken(RefreshTokenRequest("old", "oldrt"))
    ...
    fun `register socketException triggers login fallback`() = runBlocking {
        val registerSlot = slot<RegisterRequest>()
        val loginSlot = slot<LoginRequest>()
        ...
        val registerReq = RegisterRequest("testuser", "test@test.com", "Pass123!", "Test User")
